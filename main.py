from Attendance import attendpage as atd
...
def main():
    global entry_username,entry_password,window#074463
    window = tk.Tk()
    ...
    tk.Label(window,text='   EVENTIDE   ',bd=12,relief=tk.GROOVE,bg='#074463',fg='white',font=('times new roman',30,'bold')).pack(padx=20,pady=20)

    frame = tk.Frame(window, bg='#074463')  # Light blue background color
...
def login():
    ...
    if (username,) in user_arr :
        if pwd_arr[user_arr.index((username,))]==(password,):
            window.destroy()
            cursor.execute('''Select userid from user where username="{}"and password="{}"'''.format(username,password))
        ...
        # Add your code here for what to do after failed login
